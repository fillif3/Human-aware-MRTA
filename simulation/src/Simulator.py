...
people = []
den_map = 0
fname = "atc-20121212.txt"#"atc-20130213.txt"#"atc-20121226.txt"
start = 0
timestamp = 0
    ...
    if method == 'Euc':
        ...
        for i, allocation in enumerate(allocations):
            if tras is None:
                ...
                robots[allocation].addTask(tasks[i], small_map, big_map, rpoints, connections, tras[allocation][i])
            p = Path()
...
for i in range(itersMax):
    tras = None
    timeHelper = random.random() * (t_end -3600- t_ini) + t_ini
    #print(3)
